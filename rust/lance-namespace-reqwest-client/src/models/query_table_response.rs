...
/// QueryTableResponse : Query results.  This model is not used by the REST namespace, which returns the Arrow IPC file binary data directly (see the QueryTable operation). It is provided as a standard data model for non-REST LanceNamespace interfaces (e.g. Java, Python). 
