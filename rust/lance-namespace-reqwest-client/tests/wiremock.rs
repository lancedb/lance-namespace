...
//!
//! Run:   cargo test --test wiremock -- --nocapture
//! Needs: make gen-cts-wiremock  (downloads wiremock jar + generates mappings)
...
fn wiremock() -> &'static WireMockServer {
    WIREMOCK.get_or_init(|| {
        ...
        assert!(
            jar.exists(),
            "WireMock jar not found at {:?}. Run `make gen-cts-wiremock` first.",
            jar
        );
