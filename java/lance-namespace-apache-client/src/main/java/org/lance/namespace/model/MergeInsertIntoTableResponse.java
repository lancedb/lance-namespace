...
public class MergeInsertIntoTableResponse {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getNumUpdatedRows() != null) {
      try {
        joiner.add(String.format("%snum_updated_rows%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getNumUpdatedRows()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getNumInsertedRows() != null) {
      try {
        joiner.add(String.format("%snum_inserted_rows%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getNumInsertedRows()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
