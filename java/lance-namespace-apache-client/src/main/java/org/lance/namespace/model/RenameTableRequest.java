...
public class RenameTableRequest {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getNewTableName() != null) {
      try {
        joiner.add(String.format("%snew_table_name%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getNewTableName()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getNewNamespaceId() != null) {
      for (int i = 0; i < getNewNamespaceId().size(); i++) {
        try {
          joiner.add(String.format("%snew_namespace_id%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(String.valueOf(getNewNamespaceId().get(i)), "UTF-8").replaceAll("\\+", "%20")));
        } catch (UnsupportedEncodingException e) {
          // Should never happen, UTF-8 is always supported
          throw new RuntimeException(e);
        }
