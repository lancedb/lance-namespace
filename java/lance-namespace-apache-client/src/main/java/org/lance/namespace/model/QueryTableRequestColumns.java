...
public class QueryTableRequestColumns {
  ...
  public QueryTableRequestColumns columnAliases(@javax.annotation.Nullable Map<String, String> columnAliases) {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getColumnNames() != null) {
      for (int i = 0; i < getColumnNames().size(); i++) {
        try {
          joiner.add(String.format("%scolumn_names%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(String.valueOf(getColumnNames().get(i)), "UTF-8").replaceAll("\\+", "%20")));
    ...
    if (getColumnAliases() != null) {
      for (String _key : getColumnAliases().keySet()) {
        try {
          joiner.add(String.format("%scolumn_aliases%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, _key, containerSuffix),
              getColumnAliases().get(_key), URLEncoder.encode(String.valueOf(getColumnAliases().get(_key)), "UTF-8").replaceAll("\\+", "%20")));
        } catch (UnsupportedEncodingException e) {
          // Should never happen, UTF-8 is always supported
          throw new RuntimeException(e);
        }
