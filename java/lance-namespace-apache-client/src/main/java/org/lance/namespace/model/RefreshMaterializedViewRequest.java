...
public class RefreshMaterializedViewRequest {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getSrcVersion() != null) {
      try {
        joiner.add(String.format("%ssrc_version%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getSrcVersion()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getMaxRowsPerFragment() != null) {
      try {
        joiner.add(String.format("%smax_rows_per_fragment%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getMaxRowsPerFragment()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getOutputLimit() != null) {
      try {
        joiner.add(String.format("%soutput_limit%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getOutputLimit()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
