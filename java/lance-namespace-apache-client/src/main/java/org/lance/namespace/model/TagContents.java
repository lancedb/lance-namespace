...
public class TagContents {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getBranch() != null) {
      try {
        joiner.add(String.format("%sbranch%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getBranch()), "UTF-8").replaceAll("\\+", "%20")));
    ...
    if (getManifestSize() != null) {
      try {
        joiner.add(String.format("%smanifestSize%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getManifestSize()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
