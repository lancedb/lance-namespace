...
public class PartitionField {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getFieldId() != null) {
      try {
        joiner.add(String.format("%sfield_id%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getFieldId()), "UTF-8").replaceAll("\\+", "%20")));
      ...
      for (int i = 0; i < getSourceIds().size(); i++) {
        try {
          joiner.add(String.format("%ssource_ids%s%s=%s", prefix, suffix,
              ...
              URLEncoder.encode(String.valueOf(getSourceIds().get(i)), "UTF-8").replaceAll("\\+", "%20")));
        } catch (UnsupportedEncodingException e) {
          // Should never happen, UTF-8 is always supported
          throw new RuntimeException(e);
        }
