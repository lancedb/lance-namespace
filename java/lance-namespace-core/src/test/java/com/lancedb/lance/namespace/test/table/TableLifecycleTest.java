 ...
 *
 * <p>Note: All table creation operations require a location parameter to be set.
 * The location is required by the interface but not used by the server, so any value can be used.
 * For consistency in tests, we use file:// URIs pointing to /tmp/lance/ directory.
 */
