...
public class DescribeTableRequest {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getWithTableUri() != null) {
      joiner.add(String.format("%swith_table_uri%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getWithTableUri()))));
    }

    // add `load_detailed_metadata` to the URL query string
    if (getLoadDetailedMetadata() != null) {
      joiner.add(String.format("%sload_detailed_metadata%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getLoadDetailedMetadata()))));
    }

    // add `check_declared` to the URL query string
    if (getCheckDeclared() != null) {
      joiner.add(String.format("%scheck_declared%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getCheckDeclared()))));
    }

    // add `vend_credentials` to the URL query string
    if (getVendCredentials() != null) {
      joiner.add(String.format("%svend_credentials%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getVendCredentials()))));
    }
