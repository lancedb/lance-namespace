...
public class AlterTableAddColumnsRequest {
  ...
  private List<AddColumnsEntry> newColumns = new ArrayList<>();
  ...
  public AlterTableAddColumnsRequest newColumns(@javax.annotation.Nonnull List<AddColumnsEntry> newColumns) {
    ...
    if (getNewColumns() != null) {
      for (int i = 0; i < getNewColumns().size(); i++) {
        if (getNewColumns().get(i) != null) {
          joiner.add(getNewColumns().get(i).toUrlQueryString(String.format("%snew_columns%s%s", prefix, suffix,
          "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix))));
        }
      }
    }
