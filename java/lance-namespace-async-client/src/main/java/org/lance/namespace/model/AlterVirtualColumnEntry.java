...
public class AlterVirtualColumnEntry {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getImage() != null) {
      joiner.add(String.format("%simage%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getImage()))));
    }

    // add `udf` to the URL query string
    if (getUdf() != null) {
      joiner.add(String.format("%sudf%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getUdf()))));
    }

    // add `udf_name` to the URL query string
    if (getUdfName() != null) {
      joiner.add(String.format("%sudf_name%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getUdfName()))));
    }
    ...
    if (getUdfBackend() != null) {
      joiner.add(String.format("%sudf_backend%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getUdfBackend()))));
    }

    // add `auto_backfill` to the URL query string
    if (getAutoBackfill() != null) {
      joiner.add(String.format("%sauto_backfill%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getAutoBackfill()))));
    }
    ...
    if (getFieldMetadata() != null) {
      for (String _key : getFieldMetadata().keySet()) {
        joiner.add(String.format("%sfield_metadata%s%s=%s", prefix, suffix,
            ...
            getFieldMetadata().get(_key), ApiClient.urlEncode(ApiClient.valueToString(getFieldMetadata().get(_key)))));
      }
    }
