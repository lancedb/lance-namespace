...
public class FragmentSummary {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getMin() != null) {
      joiner.add(String.format("%smin%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getMin()))));
    }

    // add `max` to the URL query string
    if (getMax() != null) {
      joiner.add(String.format("%smax%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getMax()))));
    }

    // add `mean` to the URL query string
    if (getMean() != null) {
      joiner.add(String.format("%smean%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getMean()))));
    }

    // add `p25` to the URL query string
    if (getP25() != null) {
      joiner.add(String.format("%sp25%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getP25()))));
    }

    // add `p50` to the URL query string
    if (getP50() != null) {
      joiner.add(String.format("%sp50%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getP50()))));
    }

    // add `p75` to the URL query string
    if (getP75() != null) {
      joiner.add(String.format("%sp75%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getP75()))));
    }

    // add `p99` to the URL query string
    if (getP99() != null) {
      joiner.add(String.format("%sp99%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getP99()))));
    }
