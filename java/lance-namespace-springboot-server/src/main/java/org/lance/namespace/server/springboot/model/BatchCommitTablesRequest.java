...
import org.lance.namespace.server.springboot.model.CommitTableOperation;
...
@Schema(name = "BatchCommitTablesRequest", description = "Request to atomically commit a batch of table operations. This replaces `BatchCreateTableVersionsRequest` with a more general interface that supports mixed operations (DeclareTable, CreateTableVersion, DeleteTableVersions, DeregisterTable) within a single atomic transaction at the metadata layer.  All operations are committed atomically: either all succeed or none are applied. ")
...
public class BatchCommitTablesRequest {
  ...
  @Valid
  private List<@Valid CommitTableOperation> operations = new ArrayList<>();
   ...
   */
  @NotNull @Valid 
  @Schema(name = "operations", description = "List of operations to commit atomically. Supported operation types: DeclareTable, CreateTableVersion, DeleteTableVersions, DeregisterTable. ", requiredMode = Schema.RequiredMode.REQUIRED)
